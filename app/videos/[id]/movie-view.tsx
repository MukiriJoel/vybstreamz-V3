...
export default function VideoPlayer({ videoSrc = "/videos/MofayaTrailer.mp4",id}:{id:any,videoSrc:any}) {
    ...
    >
      ...
      <div 
        className={`absolute pt-2 mb-20 top-[30%] md:top-[30%] ml-4 md:ml-16 transition-opacity duration-300 ${
          showContent ? 'opacity-100 top-[10%] md:top-[15%] lg:top-[32%]' : 'opacity-0'
        }`}
      >
      ...
      {/* Fullscreen toggle button - only show when not in fullscreen and content is visible */}
        ...
        <Button
          ...
          className={`absolute bottom-4 right-4 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30 transition-all duration-300 ${
            ((showControls && isHovered) || isDragging || showContent) ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
        >
                      {isFullscreen ? (
                <Minimize className="h-5 w-5 text-white" />
  ...
  className={`absolute bottom-4 sm:bottom-6 md:bottom-8 lg:bottom-10 right-2 sm:right-4 
    ...
    showContent ? 'opacity-100 pointer-events-auto  md:bottom-32 lg:bottom-30' : 'opacity-0 pointer-events-none'
  }`}
>
  {[1, 2, 3].map((trailer) => (
